...
run((input) => {
  ...
  const parts = [c.accent(`${cfg.icons.folder} ${basename(cwd)}`)];
  const git2 = cfg.showGit ? getGitInfo(cwd) : null;
  if (git2) {
    parts.push(`${c.muted(cfg.icons.branch)} ${c.ok(git2.branch)}`);
    const divergence = [];
    if (git2.ahead > 0) divergence.push(c.accent(`${cfg.icons.ahead}${git2.ahead}`));
    if (git2.behind > 0) divergence.push(c.warn(`${cfg.icons.behind}${git2.behind}`));
