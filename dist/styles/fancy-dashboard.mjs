...
var EIGHTHS = ["", "\u258F", "\u258E", "\u258D", "\u258C", "\u258B", "\u258A", "\u2589"];
function fineBar(percentage, width) {
...
function limitRow(label, window) {
...
run((input) => {
  ...
  const windowSize = input.context_window?.context_window_size ?? 2e5;
  const ctxTail = [
  ...
  if (cfg.showCost && typeof input.cost?.total_cost_usd === "number") {
    ...
    ctxTail.push(
      c.accent(money(input.cost.total_cost_usd)) + c.muted(` ${cfg.icons.clock} ${duration(input.cost.total_duration_ms)}`) + lines
    );
  }
  const contextRow = `${gauge("ctx", ctxUsed)}  ${join(ctxTail, "  ")}`;
  const limits = cfg.showLimits ? [limitRow("5h", input.rate_limits?.five_hour), limitRow("7d", input.rate_limits?.seven_day)] : [];
